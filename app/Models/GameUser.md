...
├── role (String, default: 'player') - Rol del usuario
├── status (String, default: 'requested') - Estado del usuario 
...
| - | - | - |
| `requested`| `GameUserStatus::REQUESTED`| Usuario solicitó acceso, esperando aprobación del owner |
| `invited`| `GameUserStatus::INVITED`| Owner invitó al usuario, esperando aceptación|
...
2. Se crea registro con`status = GameUserStatus::REQUESTED`,`join_method = GameUserJoinMethod::REQUEST`
...
2. Se crea registro con`status = GameUserStatus::INVITED`,`join_method = GameUserJoinMethod::INVITATION`
